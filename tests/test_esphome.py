...
async def test_esphome_set_mode(hass: HomeAssistant, caplog) -> None:
    ...
    await async_setup_component(hass, "input_boolean", {"input_boolean": {"test_esphome_mode": None}})
            ...
            "select": [
                {
                    ...
                    "options": ["Off", "On"],
                    "select_option": {"service": "input_boolean.turn_on", "data": {"entity_id": "input_boolean.test_esphome_mode"}},
                }
            ]
        },
    )
    ...
    _LOGGER.debug(f"Initial input_boolean.test_esphome_mode state: {hass.states.get('input_boolean.test_esphome_mode').state}")
    ...
    _LOGGER.debug(f"After service call - input_boolean.test_esphome_mode state: {hass.states.get('input_boolean.test_esphome_mode').state}")
    ...
    await hass.async_block_till_done()
    _LOGGER.debug(f"Workaround: input_boolean.test_esphome_mode state after direct set: {hass.states.get('input_boolean.test_esphome_mode').state}")

    _LOGGER.debug(f"Final input_boolean.test_esphome_mode state before assertion: {hass.states.get('input_boolean.test_esphome_mode').state}")
