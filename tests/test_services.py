...
async def test_set_relay_mode(hass: HomeAssistant) -> None:
    ...
    with patch("custom_components.sun_allocator.core.services.set_mode_for_entity", new_callable=AsyncMock) as mock_call:
...
async def test_set_relay_power(hass: HomeAssistant) -> None:
    ...
    with patch("custom_components.sun_allocator.core.services.set_power_for_entity", new_callable=AsyncMock) as mock_service:
        await hass.config_entries.async_setup(config_entry.entry_id)
        await hass.async_block_till_done()
