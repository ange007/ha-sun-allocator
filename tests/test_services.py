...
async def test_set_relay_mode(hass: HomeAssistant) -> None:
    ...
    with patch("custom_components.sun_allocator.handle_set_relay_mode", new_callable=AsyncMock) as mock_call:
...
async def test_set_relay_power(hass: HomeAssistant) -> None:
    ...
    with patch("custom_components.sun_allocator.handle_set_relay_power", new_callable=AsyncMock) as mock_service:
        await hass.config_entries.async_setup(config_entry.entry_id)
        await hass.async_block_till_done()
