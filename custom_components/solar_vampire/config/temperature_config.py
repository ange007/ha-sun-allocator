"""Temperature compensation configuration module for Solar Vampire config flow."""
...
class TemperatureConfigMixin:
    ...
    def _get_temperature_config_schema(self, temperature_sensors: List[str], defaults: Optional[Dict[str, Any]] = None) -> vol.Schema:
        ...
        return vol.Schema({
            vol.Required(CONF_TEMPERATURE_SENSOR, default=default_temp_sensor): vol.In(temperature_sensors),
            vol.Required(CONF_TEMP_COEFFICIENT_VOC, default=defaults.get(CONF_TEMP_COEFFICIENT_VOC, DEFAULT_VOC_COEFFICIENT)): 
                vol.All(vol.Coerce(float), vol.Range(min=-1.0, max=0.0)),
            vol.Required(CONF_TEMP_COEFFICIENT_PMAX, default=defaults.get(CONF_TEMP_COEFFICIENT_PMAX, DEFAULT_PMAX_COEFFICIENT)): 
                vol.All(vol.Coerce(float), vol.Range(min=-1.0, max=0.0)),
        })
