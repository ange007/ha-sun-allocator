"""Solar panel configuration module for Solar Vampire config flow."""
...
class SolarConfigMixin:
    ...
    def _get_sensor_entities(self, hass: HomeAssistant) -> Dict[str, list]:
        ...
        # Add a "None" option to allow deselecting consumption
    ...
    def _get_solar_config_schema(self, sensors: Dict[str, list], defaults: Optional[Dict[str, Any]] = None) -> vol.Schema:
        ...
        return vol.Schema({
            ...
            vol.Required(CONF_PV_POWER, default=defaults.get(CONF_PV_POWER)): vol.In(sensors["power_sensors"]),
            vol.Required(CONF_PV_VOLTAGE, default=defaults.get(CONF_PV_VOLTAGE)): vol.In(sensors["voltage_sensors"]),
            vol.Required(CONF_VMP, default=defaults.get(CONF_VMP, 36.0)): vol.Coerce(float),
            vol.Required(CONF_IMP, default=defaults.get(CONF_IMP, 8.0)): vol.Coerce(float),
            vol.Optional(CONF_VOC, default=defaults.get(CONF_VOC, 44.0)): vol.Coerce(float),
            vol.Optional(CONF_ISC, default=defaults.get(CONF_ISC, 8.5)): vol.Coerce(float),
            vol.Required(CONF_PANEL_COUNT, default=defaults.get(CONF_PANEL_COUNT, 1)): vol.Coerce(int),
            vol.Required(CONF_PANEL_CONFIGURATION, default=defaults.get(CONF_PANEL_CONFIGURATION, PANEL_CONFIG_SERIES)): 
                vol.In([PANEL_CONFIG_SERIES, PANEL_CONFIG_PARALLEL, PANEL_CONFIG_PARALLEL_SERIES]),
            vol.Optional(CONF_CONSUMPTION, default=default_consumption): vol.In(sensors["consumption_sensors"]),
        })
