"""MPPT (Maximum Power Point Tracking) algorithm utilities for Solar Vampire."""
import logging
...
def calculate_current_max_power(
    ...
    if pv_voltage <= 0 or vmp <= 0:
        ...
        current_max_power = (
            pmax * light_factor * relative_voltage * current_ratio * efficiency_correction_factor
        )
        calculation_reason = "Below or at MPP"
    else:
        # Between Vmp and Voc: Use enhanced drop-off model
        current_max_power = calculate_power_above_mpp(
            pmax, light_factor, relative_voltage, voc_ratio, efficiency_correction_factor
        )
        calculation_reason = "Between Vmp and Voc"
...
def calculate_pmax(vmp: float, imp: float, panel_count: int, panel_configuration: str) -> float:
    """Calculate maximum power based on panel configuration."""
    if panel_configuration == PANEL_CONFIG_SERIES:
        # For series: Vmp is multiplied by panel_count, Imp stays the same
        return (vmp * panel_count) * imp
        ...
        return (vmp * panels_per_string) * (imp * string_count)
    else:  # parallel
        # For parallel: Vmp stays the same, Imp is multiplied by panel_count
        return vmp * (imp * panel_count)
...
def calculate_min_system_voltage(
    ...
    if panel_configuration == PANEL_CONFIG_SERIES:
        ...
        string_count = 2
        return min_inverter_voltage / string_count
    else:  # parallel
        return min_inverter_voltage / panel_count
...
def calculate_power_above_mpp(
    ...
    """Calculate power when voltage is between Vmp and Voc."""
    ...
    # Adjust drop rate based on light factor (lower light = higher drop rate)
    base_drop_rate = 1.5
    adjusted_drop_rate = base_drop_rate / light_factor
    
    # For very high voltage ratios (above 90% of Voc), increase drop rate further
    if position > 0.9:
        # Add exponential penalty for very high voltage ratios
        high_voltage_penalty = ((position - 0.9) / 0.1) ** 2
        adjusted_drop_rate += high_voltage_penalty * 2
    
    # Calculate power with adjusted drop rate
    power_factor = max(0, 1 - (adjusted_drop_rate * position ** 2))
    return pmax * light_factor * power_factor * efficiency_correction_factor
