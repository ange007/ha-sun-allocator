...
SUPPORTED_DOMAINS = {
    DOMAIN_LIGHT, DOMAIN_SWITCH, DOMAIN_INPUT_BOOLEAN, DOMAIN_AUTOMATION, DOMAIN_SCRIPT, DOMAIN_CLIMATE
}
...
def _calculate_device_state(device, remaining_power, device_on_state, device_debounce_state, cfg, now):
    ...
    log_debug(f"Device {device_name}: remaining_power={remaining_power}, on_threshold={on_threshold}, off_threshold={off_threshold}, prev_on={prev_on}, is_active_candidate={is_active_candidate}")

    # Debouncer logic
    debounce_time_s = device.get(CONF_DEBOUNCE_TIME, DEFAULT_DEBOUNCE_TIME)
    debounce_info = device_debounce_state.get(device_id, {"candidate_state": None, "state_change_time": None})
    ...
    if debounce_time_s == 0:
        ...
        elif debounce_info["state_change_time"] and (now - debounce_info["state_change_time"]).total_seconds() >= debounce_time_s:
            is_active = is_active_candidate
...
async def _control_standard_device(hass, device, is_active, prev_on, remaining_power, cfg, status_entry):
...
async def _control_custom_device(hass, device, is_active, prev_on, remaining_power, cfg, status_entry, ramp_targets):
    ...
    if mode_state.state == RELAY_MODE_PROPORTIONAL:
        ...
        power_used, status_entry = await _control_standard_device(hass, device, is_active, prev_on, remaining_power, cfg, status_entry)
    ...
    for device in auto_control_devices:
        ...
        if device.get(CONF_DEVICE_TYPE) == DEVICE_TYPE_STANDARD:
            ...
            power_used, status_entry = await _control_custom_device(
                hass, device, is_active, prev_on, remaining_power, cfg, status_entry, entry_data["ramp_targets"]
            )
